...
public class RecentInvoiceActivity extends AppCompatActivity implements OnPdfFileSelectListener {
    ...
    private void displayPdf() {
        ...
        try {
            ...
            if (SPuser.equals("admin")) {
                c1 = db.getValue("SELECT * FROM (SELECT * FROM Transation GROUP BY cus_id ) AS sorted JOIN customer ON sorted.cus_id = customer.cus_id ORDER BY sorted.time DESC LIMIT 10");
            } else {
                c1 = db.getValue("SELECT * FROM (SELECT * FROM Transation WHERE sales_user=" + "'" + SPuser + "' GROUP BY cus_id) AS sorted JOIN customer ON sorted.cus_id = customer.cus_id ORDER BY sorted.time DESC LIMIT 10");
            }
    ...
    public void onpdfSelected(File file, String mPbillno, String filename) {
        ...
        System.out.println(file.getAbsolutePath()+"123");
        try {
            if (file.exists()) {
                ...
                builder.setPositiveButton("Yes", (DialogInterface.OnClickListener) (dialog, which) -> {
                    dialog.dismiss();
                    Intent i = new Intent(RecentInvoiceActivity.this, DocumentViewActivity.class);
